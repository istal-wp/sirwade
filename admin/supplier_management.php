...
// Write operations (INSERT/UPDATE/DELETE) have been moved to staff/suppliers.php
        ...
        if (isset($_POST['action'])) {
            switch ($_POST['action']) {
                ...
                case 'update_supplier':
                    $stmt = $pdo->prepare("UPDATE suppliers SET supplier_name = ?, contact_person = ?, email = ?, phone = ?, address = ?, payment_terms = ?, tax_id = ?, status = ? WHERE id = ?");
                    $stmt->execute([
                        $_POST['supplier_name'],
                        $_POST['contact_person'],
                        $_POST['email'],
                ...
                case 'delete_supplier':
                    ...
                    if ($po_count > 0) {
                        ...
                        $stmt->execute([$_POST['supplier_id']]);
                        $message = 'Supplier deactivated (has purchase orders history)';
                        $messageType = 'warning';
                    } else {
                ...
                case 'add_evaluation':
                    $stmt = $pdo->prepare("INSERT INTO supplier_evaluations (supplier_id, evaluation_period_start, evaluation_period_end, quality_rating, delivery_rating, service_rating, price_rating, overall_rating, total_orders, on_time_deliveries, total_value, quality_issues, comments, evaluated_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                    
                    $overall_rating = ($_POST['quality_rating'] + $_POST['delivery_rating'] + $_POST['service_rating'] + $_POST['price_rating']) / 4;
                    ...
                    $stmt->execute([
                        ...
                        $_POST['delivery_rating'],
                        $_POST['service_rating'],
                        $_POST['price_rating'],
                        $overall_rating,
                        $_POST['total_orders'],
                        $_POST['on_time_deliveries'],
                        $_POST['total_value'],
                        $_POST['quality_issues'],
                        $_POST['comments'],
                        $_SESSION['user_id']
                    ]);
                    
                    $stmt = $pdo->prepare("UPDATE suppliers SET rating = ? WHERE id = ?");
                    $stmt->execute([$overall_rating, $_POST['supplier_id']]);
                    
                    $message = 'Supplier evaluation added successfully!';
                    $messageType = 'success';
                    break;
            }
        }
...
$sort = isset($_GET['sort']) ? $_GET['sort'] : 'supplier_name';
...
if ($search) {
    $where_conditions[] = "(supplier_name LIKE ? OR supplier_code LIKE ? OR contact_person LIKE ? OR email LIKE ?)";
    $search_param = "%$search%";
...
$query = "SELECT * FROM suppliers $where_clause ORDER BY $sort $order";
...
$stmt = $pdo->query("SELECT 
    ...
    SUM(CASE WHEN status = 'blacklisted' THEN 1 ELSE 0 END) as blacklisted_suppliers,
    AVG(rating) as avg_rating
FROM suppliers");
$stats = $stmt->fetch(PDO::FETCH_ASSOC);

$stmt = $pdo->query("SELECT supplier_name, rating FROM suppliers WHERE rating IS NOT NULL ORDER BY rating DESC LIMIT 5");
...
<head>
    ...
    <title>Supplier Management — BRIGHTPATH</title>
...
<style>
.top-suppliers-list { display: flex; flex-direction: column; gap: 0; }
.top-sup-row { display: flex; justify-content: space-between; align-items: center; padding: .75rem 0; border-bottom: 1px solid var(--off); }
.top-sup-row:last-child { border-bottom: none; }
.top-sup-name { font-size: .88rem; font-weight: 500; color: var(--text); }
.score-stars { color: #f59e0b; letter-spacing: 1px; font-size: .9rem; }
.score-val { font-size: .8rem; color: var(--muted); font-family: 'DM Mono', monospace; margin-left: 5px; }
</style>
</head>
<body>
<header class="header">
...
<div style="display:flex;align-items:center;gap:12px;padding:1rem 1.25rem;background:#eff6ff;border:1px solid #93c5fd;border-radius:10px;margin-bottom:1.5rem">
    ...
    <span style="font-size:.87rem;color:#1d4ed8;flex:1"><strong>Admin View:</strong> This is a read-only monitoring view. To add or modify Suppliers data, go to the Staff Portal.</span>
    <a href="../staff/suppliers.php" style="display:inline-flex;align-items:center;gap:6px;padding:.45rem .9rem;background:#2563eb;color:#fff;border-radius:8px;font-size:.82rem;font-weight:600;text-decoration:none">
  ...
  <div class="page-title"><h1>Supplier Management</h1><p>Manage supplier relationships, evaluations, and performance tracking</p></div>

  <?php if ($message): ?>
  <div class="alert alert-<?php echo $messageType === 'success' ? 'success' : 'error'; ?>">
    <svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><?php echo $messageType === 'success' ? '<path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/>' : '<circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/>'; ?></svg>
  ...
  <div class="stats-grid">
    <div class="stat-card">
      <div class="stat-top"><span class="stat-label">Total Suppliers</span><div class="stat-badge"><svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg></div></div>
      <div class="stat-value"><?php echo number_format($stats['total_suppliers']); ?></div>
      <div class="stat-sub">All registered suppliers</div>
    </div>
    <div class="stat-card">
      <div class="stat-top"><span class="stat-label">Active Suppliers</span><div class="stat-badge success-badge"><svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg></div></div>
      <div class="stat-value"><?php echo number_format($stats['active_suppliers']); ?></div>
      <div class="stat-sub good">Currently active</div>
    </div>
    <div class="stat-card">
      <div class="stat-top"><span class="stat-label">Inactive Suppliers</span><div class="stat-badge warn-badge"><svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></svg></div></div>
      <div class="stat-value"><?php echo number_format($stats['inactive_suppliers']); ?></div>
      <div class="stat-sub warn">Temporarily disabled</div>
    </div>
    <div class="stat-card">
      <div class="stat-top"><span class="stat-label">Average Rating</span><div class="stat-badge warn-badge"><svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg></div></div>
  ...
  <div class="panel">
    ...
    <form method="GET" style="display:flex;gap:.65rem;flex-wrap:wrap;margin-bottom:1.25rem">
      <div class="field" style="flex:2;min-width:180px"><input type="text" name="search" class="form-control" placeholder="Search suppliers…" value="<?php echo htmlspecialchars($search); ?>"></div>
      <div class="field" style="flex:1;min-width:140px"><select name="status" class="form-control"><option value="">All Status</option><option value="active"<?php echo $status_filter === 'active' ? ' selected' : ''; ?>>Active</option><option value="inactive"<?php echo $status_filter === 'inactive' ? ' selected' : ''; ?>>Inactive</option><option value="blacklisted"<?php echo $status_filter === 'blacklisted' ? ' selected' : ''; ?>>Blacklisted</option></select></div>
    ...
    <div style="overflow-x:auto">
      <table class="data-table">
        <thead><tr>
          <th><a href="?sort=supplier_code&order=<?php echo $sort === 'supplier_code' && $order === 'ASC' ? 'DESC' : 'ASC'; ?>&search=<?php echo $search; ?>&status=<?php echo $status_filter; ?>" style="color:inherit;text-decoration:none">Code</a></th>
          <th><a href="?sort=supplier_name&order=<?php echo $sort === 'supplier_name' && $order === 'ASC' ? 'DESC' : 'ASC'; ?>&search=<?php echo $search; ?>&status=<?php echo $status_filter; ?>" style="color:inherit;text-decoration:none">Supplier Name</a></th>
          <th>Contact Person</th><th>Email</th><th>Phone</th><th>Payment Terms</th><th>Rating</th><th>Status</th><th>Actions</th>
        </tr></thead>
        <tbody>
          <?php if (empty($suppliers)): ?>
          <tr><td colspan="9">
            <div class="empty-state">
              <svg viewBox="0 0 24 24" fill="none" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/></svg>
              <h3>No suppliers found</h3><p>Add your first supplier or adjust search criteria.</p>
            </div>
          </td></tr>
          <?php else: ?>
          <?php foreach ($suppliers as $s): ?>
          <tr>
            <td><strong style="font-family:'DM Mono',monospace;font-size:.82rem"><?php echo htmlspecialchars($s['supplier_code']); ?></strong></td>
            <td><div><strong><?php echo htmlspecialchars($s['supplier_name']); ?></strong><br><small style="color:var(--muted);font-size:.75rem"><?php echo htmlspecialchars($s['address']); ?></small></div></td>
            <td><?php echo htmlspecialchars($s['contact_person']); ?></td>
            <td style="font-size:.82rem"><?php echo htmlspecialchars($s['email']); ?></td>
            <td><?php echo htmlspecialchars($s['phone']); ?></td>
            <td><?php echo htmlspecialchars($s['payment_terms']); ?></td>
            <td>
              <?php if ($s['rating']): ?>
              <span class="score-stars"><?php $r = floatval($s['rating']); for ($i=1; $i<=5; $i++) echo $i <= $r ? '★' : '☆'; ?></span>
              <span class="score-val">(<?php echo number_format(floatval($s['rating']), 1); ?>)</span>
              <?php else: ?><span style="color:var(--muted);font-size:.82rem">Not rated</span><?php endif; ?>
            </td>
            <td><span class="badge status-<?php echo $s['status']; ?>"><?php echo ucfirst($s['status']); ?></span></td>
            <td>
              <div style="display:flex;gap:.4rem;flex-wrap:wrap">
                <button class="btn btn-primary btn-sm" onclick="editSupplier(<?php echo htmlspecialchars(json_encode($s)); ?>)">
                ...
                <button class="btn btn-outline btn-sm" onclick="openEvaluationModal(<?php echo $s['id']; ?>, '<?php echo htmlspecialchars($s['supplier_name']); ?>')">
                ...
                <button class="btn btn-danger btn-sm" onclick="confirmDelete(<?php echo $s['id']; ?>, '<?php echo htmlspecialchars($s['supplier_name']); ?>')">
                  <svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/></svg>
                  Delete
                </button>
              </div>
            </td>
  ...
  <div class="panel">
    ...
    <div class="top-suppliers-list">
      ...
      <div class="top-sup-row">
        <span class="top-sup-name"><?php echo htmlspecialchars($ts['supplier_name']); ?></span>
        <div>
          <span class="score-stars"><?php $r = floatval($ts['rating']); for ($i=1; $i<=5; $i++) echo $i <= $r ? '★' : '☆'; ?></span>
          <span class="score-val"><?php echo number_format($r, 1); ?></span>
        </div>
      </div>
      <?php endforeach; ?>
    </div>
...
<div id="addModal" class="modal">
  <div class="modal-box">
    <div class="modal-head"><h3>Add New Supplier</h3><button class="modal-close" onclick="closeModal('addModal')">&times;</button></div>
    <div class="modal-body">
      <form method="POST">
        <input type="hidden" name="action" value="add_supplier">
        <div class="form-grid-2">
          <div class="form-field"><label>Supplier Name *</label><input type="text" name="supplier_name" class="form-control" required></div>
          <div class="form-field"><label>Contact Person *</label><input type="text" name="contact_person" class="form-control" required></div>
          <div class="form-field"><label>Email *</label><input type="email" name="email" class="form-control" required></div>
          <div class="form-field"><label>Phone</label><input type="text" name="phone" class="form-control"></div>
          <div class="form-field"><label>Payment Terms</label><select name="payment_terms" class="form-control"><option value="">Select…</option><option>Cash on Delivery</option><option>Net 30</option><option>Net 60</option><option>Net 90</option><option>Advance Payment</option><option>50% Advance, 50% on Delivery</option></select></div>
          <div class="form-field"><label>Tax ID</label><input type="text" name="tax_id" class="form-control"></div>
          <div class="form-field"><label>Status *</label><select name="status" class="form-control" required><option value="active">Active</option><option value="inactive">Inactive</option><option value="blacklisted">Blacklisted</option></select></div>
        </div>
        <div class="form-field"><label>Address</label><textarea name="address" class="form-control"></textarea></div>
...
<div id="editModal" class="modal">
  <div class="modal-box">
    <div class="modal-head"><h3>Edit Supplier</h3><button class="modal-close" onclick="closeModal('editModal')">&times;</button></div>
    <div class="modal-body">
      <form method="POST">
        <input type="hidden" name="action" value="update_supplier">
        <input type="hidden" name="supplier_id" id="edit_supplier_id">
        <div class="form-grid-2">
          <div class="form-field"><label>Supplier Name *</label><input type="text" id="edit_supplier_name" name="supplier_name" class="form-control" required></div>
          <div class="form-field"><label>Contact Person *</label><input type="text" id="edit_contact_person" name="contact_person" class="form-control" required></div>
          <div class="form-field"><label>Email *</label><input type="email" id="edit_email" name="email" class="form-control" required></div>
          <div class="form-field"><label>Phone</label><input type="text" id="edit_phone" name="phone" class="form-control"></div>
          <div class="form-field"><label>Payment Terms</label><select id="edit_payment_terms" name="payment_terms" class="form-control"><option value="">Select…</option><option>Cash on Delivery</option><option>Net 30</option><option>Net 60</option><option>Net 90</option><option>Advance Payment</option><option>50% Advance, 50% on Delivery</option></select></div>
          <div class="form-field"><label>Tax ID</label><input type="text" id="edit_tax_id" name="tax_id" class="form-control"></div>
          <div class="form-field"><label>Status *</label><select id="edit_status" name="status" class="form-control" required><option value="active">Active</option><option value="inactive">Inactive</option><option value="blacklisted">Blacklisted</option></select></div>
        </div>
        <div class="form-field"><label>Address</label><textarea id="edit_address" name="address" class="form-control"></textarea></div>
        <div class="modal-footer">
...
<div id="evaluationModal" class="modal">
  <div class="modal-box" style="max-width:640px">
    <div class="modal-head"><h3 id="eval_modal_title">Supplier Evaluation</h3><button class="modal-close" onclick="closeModal('evaluationModal')">&times;</button></div>
    <div class="modal-body">
      <form method="POST">
        <input type="hidden" name="action" value="add_evaluation">
        <input type="hidden" name="supplier_id" id="eval_supplier_id">
        <div class="form-grid-2">
          <div class="form-field"><label>Period Start *</label><input type="date" name="period_start" class="form-control" required></div>
          <div class="form-field"><label>Period End *</label><input type="date" name="period_end" class="form-control" required></div>
          <div class="form-field"><label>Quality Rating (1–5) *</label><select name="quality_rating" class="form-control" required><option value="">Select…</option><option value="1">1 — Poor</option><option value="2">2 — Fair</option><option value="3">3 — Good</option><option value="4">4 — Very Good</option><option value="5">5 — Excellent</option></select></div>
          <div class="form-field"><label>Delivery Rating (1–5) *</label><select name="delivery_rating" class="form-control" required><option value="">Select…</option><option value="1">1 — Poor</option><option value="2">2 — Fair</option><option value="3">3 — Good</option><option value="4">4 — Very Good</option><option value="5">5 — Excellent</option></select></div>
          <div class="form-field"><label>Service Rating (1–5) *</label><select name="service_rating" class="form-control" required><option value="">Select…</option><option value="1">1 — Poor</option><option value="2">2 — Fair</option><option value="3">3 — Good</option><option value="4">4 — Very Good</option><option value="5">5 — Excellent</option></select></div>
          <div class="form-field"><label>Price Rating (1–5) *</label><select name="price_rating" class="form-control" required><option value="">Select…</option><option value="1">1 — Poor</option><option value="2">2 — Fair</option><option value="3">3 — Good</option><option value="4">4 — Very Good</option><option value="5">5 — Excellent</option></select></div>
          <div class="form-field"><label>Total Orders</label><input type="number" name="total_orders" class="form-control" min="0"></div>
          <div class="form-field"><label>On-Time Deliveries</label><input type="number" name="on_time_deliveries" class="form-control" min="0"></div>
          <div class="form-field"><label>Total Value</label><input type="number" name="total_value" class="form-control" min="0" step="0.01"></div>
          <div class="form-field"><label>Quality Issues</label><input type="number" name="quality_issues" class="form-control" min="0"></div>
        </div>
        <div class="form-field"><label>Comments</label><textarea name="comments" class="form-control" rows="3"></textarea></div>
        <div class="modal-footer">
          <button type="button" onclick="closeModal('evaluationModal')" class="btn btn-outline">Cancel</button>
          <button type="submit" class="btn btn-success">
            <svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg>
            Save Evaluation
          </button>
...
<div id="deleteModal" class="modal">
  <div class="modal-box" style="max-width:420px">
    <div class="modal-head"><h3>Confirm Delete</h3><button class="modal-close" onclick="closeModal('deleteModal')">&times;</button></div>
    <div class="modal-body">
      <p style="color:var(--text);margin-bottom:.75rem">Are you sure you want to delete this supplier?</p>
      <p><strong id="delete_supplier_name"></strong></p>
      <div class="alert alert-warn" style="margin-top:1rem">
        ...
        <span>If this supplier has purchase orders, it will be deactivated instead of deleted.</span>
      </div>
      <form method="POST">
        ...
        <div class="modal-footer">
          ...
          <button type="submit" class="btn btn-danger">
            <svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/></svg>
            Delete
          </button>
        </div>
      </form>
...
function openEvaluationModal(id, name) {
    document.getElementById('eval_supplier_id').value = id;
    document.getElementById('eval_modal_title').textContent = 'Evaluate: ' + name;
